...
export default function App() {
  ...
  async function onLogin(e) {
    ...
    try {
      ...
      await loadMe();
      // Если вошёл admin — подгрузим список пользователей
      await loadUsers();
    } catch (e2) {
      setAuthError(String(e2?.response?.data?.message || e2?.message || e2));
    } finally {
...
// --- PRODUCTS state ---
  ...
  async function onAdd(e) {
    ...
    if (!canSubmit) return;
    if (!isAdmin) {
      setError("Только admin может создавать товары (Практика 11).");
      return;
    }
        ...
        <main style={s.grid}>
          ...
          <section style={s.card}>
            ...
            <form onSubmit={onAdd} style={{ ...s.row, marginTop: 10 }}>
              ...
              />
              <button disabled={!canSubmit || !isAdmin} style={s.btn("primary", !canSubmit)}>
                Создать
              </button>
            </form>
              ...
              {items.map((p) => (
                <li key={p.id} style={s.item}>
                  ...
                  <div style={s.itemActions}>
                    <button onClick={() => onPricePlus(p.id, p.price)} style={s.btn("secondary")} disabled={!isAdmin}>
                      +10 ₽ (PUT)
                    </button>
                    <button onClick={() => onDelete(p.id)} style={s.btn("danger")} disabled={!isAdmin}>
                      Удалить
                    </button>
                  </div>
                </li>
